...
temp <- merged_wgps %>%
  ...
  summarise_at(.vars = c("rdtPos","micPos"), 
               funs(Sum = sum(.,na.rm=TRUE),Na = sum(is.na(.)), tested = sum(!is.na(.))))
...
household_lvl_data <- temp11 %>%
  mutate(oneNet = ifelse(!is.na(num_net), 1, 0), # at least one net in household
         ...
         rdtRate = rdtPos_Sum/rdtPos_tested, # rdt rate: total # rdt + in house / total # ppl tested in hh
         micRate = micPos_Sum/micPos_tested, # mic rate: total # mic + in house / total # ppl tested in hh
...
# More processing in raw/manage_modis.R file 
